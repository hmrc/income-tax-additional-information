...
class GainsPriorDataModelSpec extends TestSuite {
  ...
  "GainsPriorDataModel" should {
    ...
    "correctly parse to a model" when {
      "the json contains all the data for the model" in {
        jsonMax.as[GainsPriorDataModel] mustBe modelMax
      }

      "the json contains no data" in {
        jsonMin.as[GainsPriorDataModel] mustBe modelMin
      }
    }
    ...
    "correctly map to PolicyCyaModel" when {
      "each prior model has data" in {
        ...
        val lifeInsuranceModel: PolicyCyaModel =
        ...
        result mustBe
          List(
            ...
            lifeInsuranceModel.copy(sessionId = result(4).sessionId, policyType =
              Some("Foreign Policy"), policyEvent = Some(""), yearsPolicyHeldPrevious = Some(0), treatedAsTaxPaid = Some(false), deficiencyReliefAmount = Some(0)
            )
          )
      }
    }

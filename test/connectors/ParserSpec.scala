...
class ParserSpec extends UnitTest {
  ...
  "TestParser" should {
    ...
    "handle multiple errors" in {
      ...
      result shouldBe Left(errors.ApiError(INTERNAL_SERVER_ERROR, MultiErrorsBody(Seq(
    ...
    "handle single errors" in {
      ...
      val result = underTest.handleError(response, response.status)
      result shouldBe Left(errors.ApiError(INTERNAL_SERVER_ERROR, SingleErrorBody("INTERNAL_SERVER_ERROR", "The service is currently facing issues.")))
    }

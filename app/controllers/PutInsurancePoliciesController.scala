...
class PutInsurancePoliciesController @Inject()(putInsurancePoliciesService: PutInsurancePoliciesService,
  ...
  def putInsurancePolicies(nino: String, taxYear: Int): Action[AnyContent] = authorisedAction.async { implicit user =>
    user.request.body.asJson.map(_.validate[CreateOrAmendInsurancePoliciesModel]) match {
      case Some(JsSuccess(model, _)) => responseHandler(putInsurancePoliciesService.putInsurancePolicies(nino, taxYear, model))
      case _ => Future.successful(BadRequest)
    }
  }
    def responseHandler(serviceResponse: Future[PutInsurancePoliciesResponse]): Future[Result] = {
      serviceResponse.map {
